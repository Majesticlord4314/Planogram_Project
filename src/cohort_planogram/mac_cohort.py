...
This module generates detailed cohort-based planograms for Mac,
showing relationships between Mac models and their accessories
...
class MacCohortPlanogram(CohortPlanogramBase):
    ...
    def generate_cohort_planogram(self, store_type: str) -> Path:
        ...
        # Get top Mac models and accessory categories based on store type
        matrix_config = self.store_template_loader.get_matrix_config(store_type)
        top_models = self.data_loader.get_top_core_products('Mac', limit=matrix_config['max_models'])
        top_categories = self.data_loader.get_top_accessory_categories('Mac', limit=matrix_config['max_categories'])
        
        # Get cohort matrices
        attach_rate_matrix = self.data_loader.get_cohort_matrix('Mac', top_models, top_categories)
        frequency_matrix = self.data_loader.get_frequency_matrix('Mac', top_models, top_categories)
        ...
        # Get insights
        top_cohort_pairs = self.data_loader.get_top_cohort_pairs('Mac', limit=8)
        summary_stats = self.data_loader.get_lob_summary_stats('Mac')
        ...
        # Add title
        self.add_title(ax, f"Mac Cohort Planogram - {store_type.title()} Store", y_pos=layout_positions['title_y'])
        
        # Create core product zones (store-specific sizing)
        self._create_core_product_zones(ax, mac_data, top_models, store_type, **core_product_config)
        
        # Create cohort matrix (main section, store-specific)
        self._create_cohort_matrix(ax, attach_rate_matrix, frequency_matrix, mac_data, store_type, y_start=layout_positions['matrix_y'])
    ...
    def _create_core_product_zones(self, ax, mac_data: pd.DataFrame, 
                                  top_models: List[str], store_type: str,
                                  zone_width: int, zone_height: int, 
                                  x_start: int, y_start: int, x_spacing: int) -> None:
        """Create Mac core product zones with performance indicators"""
    ...
    def _create_cohort_matrix(self, ax, attach_rate_matrix: pd.DataFrame, 
                             frequency_matrix: pd.DataFrame, mac_data: pd.DataFrame, 
                             store_type: str, y_start: int) -> None:
        """Create clean cohort matrix showing Mac models vs accessories"""
        ...
        # Add column headers (Mac models) - properly aligned
        header_y = y_start - 5
        ...
        for row, category in enumerate(categories):
            ...
            for col, model in enumerate(models):
                ...
                # Get attach rate and frequency with better error handling
                try:
                    attach_rate = attach_rate_matrix.loc[category, model]
                    frequency = frequency_matrix.loc[category, model]
                    
                    # Handle NaN values
                    if pd.isna(attach_rate) or pd.isna(frequency):
                        attach_rate = 0
                        frequency = 0
                ...
                if attach_rate > 0:
                    ...
                    top_seller = self._get_top_seller_for_category(mac_data, category)
                    if top_seller:
    ...
    def _get_top_seller_for_category(self, mac_data: pd.DataFrame, category: str) -> Dict:
        """Get the top-selling product for a specific category"""
        try:
            category_data = mac_data[mac_data['accessory_category'] == category]
    ...
    def _add_recommended_layout(self, ax, top_models: List[str], 
        ...
        """Add recommended planogram layout for Mac based on cohort insights"""
        ...
        # Define shelf layout for Mac (eye level = middle shelves)
        shelves = [
            {'name': 'Top Shelf', 'products': ['Mac sleeves', 'Storage'], 'color': '#E5E5EA'},
            {'name': 'Eye Level 1', 'products': ['MacBook Pro 16"', 'MacBook Pro 14"'], 'color': '#007AFF'},
            {'name': 'Eye Level 2', 'products': ['Keyboards', 'Mice & trackpads'], 'color': '#34C759'},
            {'name': 'Mid Level', 'products': ['MacBook Air', 'iMac accessories'], 'color': '#FF9500'},
            {'name': 'Bottom Shelf', 'products': ['Hubs', 'Cables & adapters'], 'color': '#8E8E93'}
        ]
        ...
        principles = [
            "Premium models at eye level",
            "Keyboards & mice nearby",
            "Hubs for productivity",
    ...
    def _generate_product_list(self, top_models: List[str], top_categories: List[str], 
        ...
        self.logger.info("Generating detailed product list for Mac cohort combinations")
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            for model in top_models:
                ...
                for category in top_categories:
                    ...
                    f.write(f"\n{category_emoji} {category} (Avg Attach Rate: {avg_attach_rate:.1%})\n")
            ...
            top_combinations = mac_data[
                (mac_data['core_product'].isin(top_models)) &
                (mac_data['accessory_category'].isin(top_categories))
            ...
            for category in top_categories:
                category_data = mac_data[
                    (mac_data['core_product'].isin(top_models)) &
                    (mac_data['accessory_category'] == category)
                ]
