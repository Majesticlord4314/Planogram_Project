...
This module generates detailed cohort-based planograms for iPad,
showing relationships between iPad models and their accessories
...
class iPadCohortPlanogram(CohortPlanogramBase):
        ...
        for row, category in enumerate(categories):
            ...
            for col, model in enumerate(models):
                ...
                if attach_rate > 0:
                    ...
                    top_seller = self._get_top_seller_for_category(ipad_data, category)
                    if top_seller:
    ...
    def _get_top_seller_for_category(self, ipad_data: pd.DataFrame, category: str) -> Dict:
    ...
    def _add_recommended_layout(self, ax, top_models: List[str], 
        ...
        """Add recommended planogram layout for iPad based on cohort insights"""
        ...
        # Define shelf layout for iPad (eye level = middle shelves)
        shelves = [
            {'name': 'Top Shelf', 'products': ['iPad cases', 'Seasonal accessories'], 'color': '#E5E5EA'},
            {'name': 'Eye Level 1', 'products': ['iPad Pro 12.9"', 'iPad Pro 11"'], 'color': '#007AFF'},
            {'name': 'Eye Level 2', 'products': ['Apple Pencil', 'Keyboards'], 'color': '#34C759'},
            {'name': 'Mid Level', 'products': ['iPad Air', 'iPad mini'], 'color': '#FF9500'},
            {'name': 'Bottom Shelf', 'products': ['Stands', 'Cables & adapters'], 'color': '#8E8E93'}
        ]
    ...
    def _generate_product_list(self, top_models: List[str], top_categories: List[str], 
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            top_combinations = ipad_data[
                (ipad_data['core_product'].isin(top_models)) &
                (ipad_data['accessory_category'].isin(top_categories))
            ...
            for category in top_categories:
                category_data = ipad_data[
                    (ipad_data['core_product'].isin(top_models)) &
                    (ipad_data['accessory_category'] == category)
                ]
